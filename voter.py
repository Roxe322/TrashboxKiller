...
API_LINK = MAIN_LINK + 'ajax.php'
session = requests.Session()

"""
Create 2 files in script directory, logins.txt with lines in format login:password and comment_ids.txt with lines of comments ids (only numbers)
"""
...
def auth(login, password):
	...
	auth_request = session.post(API_LINK, data=auth_data)
	return get_ajax_code()


def vote_comment(ajax_code, comment_id):
	vote_data = {
		'mycat': '234',
		'type': '1',
		'action': 'vote',
	...
	free_votes = int(float(me_data[13]) // 0.3)
	print('Account ' + account_name + ' can put ' + str(free_votes) + ' votes')
	if free_votes < 1:
		print('No free votes')
		return False
	else:
	...
	with open(db_path, 'r') as users:
		for user in users.readlines():
			...
			if len(user_auth_info) != 2: # if string is invalid
				...
				continue
			login = user_auth_info[0].strip()
			password = user_auth_info[1].strip()
